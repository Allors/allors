...
public class PushResponseBuilder
{
                ...
                foreach (var pushRequestNewObject in pushRequest.n.OrderByDescending(v => v.w))
                {
                    ...
                    if (pushRequestRoles != null)
                    {
                        countOutstandingRoles +=
                            this.PushRequestRoles(pushRequestRoles, obj, pushResponse, objectByNewId, newIdByObject, true);
                    }
                }
    ...
    private int PushRequestRoles(IList<PushRequestRole> pushRequestRoles, IObject obj, PushResponse pushResponse,
        Dictionary<long, IObject> objectByNewId, Dictionary<IObject, long> newIdByObject, bool ignore = false)
    {
        var countOutstandingRoles = 0;
