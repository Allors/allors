...
public abstract class ServicesTest : IDisposable
{
    ...
    public void PrefetchCompositeRoleOne2One()
    {
        foreach (var twice in TrueFalse)
        {
            foreach (var init in this.Inits)
            {
                ...
                this.Transaction.Prefetch(new RelationEndType[] { m.C1.C1C2one2one }, new[] { c1A.Strategy.ObjectId });
                if (twice)
                {
                    this.Transaction.Prefetch(new RelationEndType[] { m.C1.C1C2one2one }, new[] { c1A.Strategy.ObjectId });
                }
    ...
    public void PrefetchCompositesRoleOne2ManyEmpty()
    {
        foreach (var twice in TrueFalse)
        {
            foreach (var init in this.Inits)
            {
                ...
                this.Transaction.Prefetch(new RelationEndType[] { m.C1.C1C2one2manies }, c1A);
                if (twice)
                {
                    this.Transaction.Prefetch(new RelationEndType[] { m.C1.C1C2one2manies }, c1A);
                }
    ...
    public void PrefetchCompositesRoleMany2ManyEmpty()
    {
        foreach (var twice in TrueFalse)
        {
            foreach (var init in this.Inits)
            {
                ...
                this.Transaction.Prefetch(new RelationEndType[] { m.C1.C1C2many2manies }, c1A);
                if (twice)
                {
                    this.Transaction.Prefetch(new RelationEndType[] { m.C1.C1C2many2manies }, c1A);
                }
    ...
    public void PrefetchAssociationOne2OneEmpty()
    {
        foreach (var twice in TrueFalse)
        {
            foreach (var init in this.Inits)
            {
                ...
                this.Transaction.Prefetch(new RelationEndType[] { m.C2.C1WhereC1C2one2one }, c2A);
                if (twice)
                {
                    this.Transaction.Prefetch(new RelationEndType[] { m.C2.C1WhereC1C2one2one }, c2A);
                }
    ...
    public void PrefetchAssociationMany2ManyEmpty()
    {
        foreach (var twice in TrueFalse)
        {
            foreach (var init in this.Inits)
            {
                ...
                this.Transaction.Prefetch(new RelationEndType[] { m.C2.C1sWhereC1C2many2many }, c2A);
                if (twice)
                {
                    this.Transaction.Prefetch(new RelationEndType[] { m.C2.C1sWhereC1C2many2many }, c2A);
                }
    ...
    public void PrefetchAssociationMany2OneEmpty()
    {
        foreach (var twice in TrueFalse)
        {
            foreach (var init in this.Inits)
            {
                ...
                this.Transaction.Prefetch(new RelationEndType[] { m.C2.C1sWhereC1C2many2one }, c2A);
                if (twice)
                {
                    this.Transaction.Prefetch(new RelationEndType[] { m.C2.C1sWhereC1C2many2one }, c2A);
                }
    ...
    private IObject[] GetExtent(Composite objectType) => this.Transaction.Extent(objectType);
}
