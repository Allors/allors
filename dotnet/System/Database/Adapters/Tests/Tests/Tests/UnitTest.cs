...
public abstract class UnitTest : IDisposable
{
    ...
    public void AllorsBooleanWeakReference()
    {
        foreach (var init in this.Inits)
        {
            ...
            if (this.Transaction is ITransaction)
            {
                ...
                extent.AddEquals(m.C1.C1AllorsBoolean, true);
    ...
    public virtual void AllorsDateTimeWeakReference()
    {
        foreach (var init in this.Inits)
        {
            ...
            if (this.Transaction is ITransaction)
            {
                ...
                // Force a Flush
                var extent = this.Transaction.Filter<C1>();
                extent.AddEquals(m.C1.C1AllorsDateTime, new DateTime(1973, 03, 27, 1, 2, 3, 4, DateTimeKind.Utc));
    ...
    public virtual void AllorsDecimalWeakReference()
    {
        foreach (var init in this.Inits)
        {
            ...
            if (this.Transaction is ITransaction)
            {
                ...
                // Force a Flush
                var extent = this.Transaction.Filter<C1>();
                extent.AddEquals(m.C1.C1AllorsDecimal, 1M);
                Assert.NotNull(extent.First());
    ...
    public void AllorsLargeStringWeakReference()
    {
        foreach (var init in this.Inits)
        {
            ...
            if (this.Transaction is ITransaction)
            {
                ...
                // Force a Flush
                var extent = this.Transaction.Filter<C1>();
                extent.AddEquals(m.C1.C1StringLarge, aLarge);
                Assert.NotNull(extent.First());
    ...
    public void AllorsSmallBinaryWeakReference()
    {
        foreach (var init in this.Inits)
        {
            ...
            if (this.Transaction is ITransaction)
            {
                ...
                // Force a Flush
                var extent = this.Transaction.Filter<C1>();
                extent.AddEquals(m.C1.C1AllorsBinary, binary1);
                Assert.NotNull(extent.First());
    ...
    public void AllorsSmallStringWeakReference()
    {
        foreach (var init in this.Inits)
        {
            ...
            if (this.Transaction is ITransaction)
            {
                ...
                // Force a Flush
                var extent = this.Transaction.Filter<C1>();
                extent.AddEquals(m.C1.C1AllorsString, "a");
                Assert.NotNull(extent.First());
    ...
    public virtual void AllorsUniqueWeakReference()
    {
        foreach (var init in this.Inits)
        {
            ...
            if (this.Transaction is ITransaction)
            {
                ...
                // Force a Flush
                var extent = this.Transaction.Filter<C1>();
                extent.AddEquals(m.C1.C1AllorsUnique, unique);
                Assert.NotNull(extent.First());
    ...
    protected DateTime StripNanoSeconds(DateTime dateTime) => new(dateTime.Year, dateTime.Month, dateTime.Day, dateTime.Hour,
