namespace Allors.Embedded.Domain.Tests
{
    ...
    public abstract class ObjectsTests : Tests
    {
        ...
        public void Filter()
        {
            ...
            var lastNameDoe = this.Population.EmbeddedObjects.OfType<Person>().Where(v => v.LastName.Value == "Doe").ToArray();
            ...
            var lessThanFourLetterFirstNames = this.Population.EmbeddedObjects.OfType<Person>().Where(v => v.FirstName.Value.Length < 4).ToArray();

            Assert.That(lessThanFourLetterFirstNames, Is.Empty);

            var fourLetterFirstNames = this.Population.EmbeddedObjects.OfType<Person>().Where(v => v.FirstName.Value.Length == 4).ToArray();
            ...
            var fiveLetterFirstNames = this.Population.EmbeddedObjects.OfType<Person>().Where(v => v.FirstName.Value.Length == 5).ToArray();
